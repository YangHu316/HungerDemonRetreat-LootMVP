...
| [test/unit/test_monster.gd](test/unit/test_monster.gd) §07 部分 | **§07 寻路 — NavigationAgent3D**:Monster 用 NavigationAgent3D 替代 _wall_raycast(支持跨房间 / 绕容器);_move_toward 查 target_position / get_next_path_position / is_navigation_finished(没 navmesh fallback 直走);main.tscn 加 NavigationRegion3D 配 source_group_name=navigation_geometry;main.gd._setup_navigation 把 Foundation 加 group + 调 bake_navigation_mesh;_add_wall 把墙加 navigation_geometry group;container.gd._ready 把容器加 group(障碍);门不加 group(烘焙忽略 → 路径可穿门道);**stuck 兜底**(警觉态有 velocity 但实际位移 < 0.05m 累计 3s 切 RETURNING,防关门死等);CHASE_LOSE_TIME 1.5s(失视线 snappy,不能用 spec 10s 那是 SEARCH 内 give-up);_check_stuck 调用必须在 match state 之后 / move_and_slide 之前(否则 velocity 已被物理归零) |
| [test/unit/test_phase3b_sound_coverage.gd](test/unit/test_phase3b_sound_coverage.gd) | **§06 Phase 3B 发声覆盖**(spec 05§1.2):container.open() 加 _emit_search_sound(drawer/cabinet 4m / safe 8m);door._apply_toggle_local 加 _emit_door_sound(8m);player 订阅 item_moved 加 _on_item_moved 在 to_grid_id=inventory 且 from_grid_id≠inventory 时 emit PICKUP_SOUND_RADIUS=1.5m;三个 emit 都查 is_single 跳过多人(host 权威 Phase 2C 再做) |

跑全量 = 365 测试 / 1043 assert。
